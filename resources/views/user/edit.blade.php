...
<div class="container">
    ...
    <form action="{{ route('user.update', $user->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" name="nama" value="{{ $user->name ?? old('nama') }}" required>
        </div>
        <div class="mb-3">
            <label for="nama" class="form-label">NIP</label>
            <input type="text" class="form-control" name="nip" value="{{ $user->nip ?? old('nip') }}" required>
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <select class="form-select" aria-label="Default select example" name="role" id="role">
                ...
                @foreach($role as $j)
                <option value="{{ $j->id }}" @selected( $user->role_id == $j->id )>{{ $j->role }}</option>
                @endforeach
            </select>
        </div>
